function predRGB = Probe(config_file, ImName) %, Dm)
%%%%%%%%%%%%%%%%%%%%
% Testing STF just for pixel level output on just one image
...
%numTest = numel(ImName);
%wait = waitbar(0, 'testing');
%fprintf('start testing\n');
%totalTime = 0;

    tic;
    patches = getPatches([ImName], DIR, [], BOX, []);   
...
%    totalTime = totalTime + toc;
    ...
    title('prediction');
    print(h, '-dpng', fullfile(DIR.result, regexprep(ImName, '\.jpg$', '.png')))
    end
    %%
